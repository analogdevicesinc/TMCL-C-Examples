...
int32_t readRegisterUART(uint16_t icID, uint8_t registerAddress)
{
	...
	data[0] = 0x05;
	data[1] = tmc5272_getNodeAddress(icID); //targetAddressUart;
...
void writeRegisterUART(uint16_t icID, uint8_t registerAddress, int32_t value)
{
	...
	data[0] = 0x05;
	data[1] = (uint8_t)tmc5272_getNodeAddress(icID); //targetAddressUart;
...
void tmc5272_rotateMotor(uint16_t icID, uint8_t motor, int32_t velocity)
{
  if(motor >= TMC5272_MOTORS)
	...
	tmc5272_writeRegister(icID, TMC5272_VMAX(motor), abs(velocity));
